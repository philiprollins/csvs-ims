...
public class RecountPartCommand : ICommand
{
    ...
    private RecountPartCommand(PartSku sku, Quantity quantity, string justification)
    {
        Sku = sku;
        Quantity = quantity;
    ...
    public static Result<RecountPartCommand> Create(string sku, int quantity, string justification)
    {
        ...
        return Result.Ok(new RecountPartCommand(skuResult.Value, quantityResult.Value, justification.Trim()));
    }
}
